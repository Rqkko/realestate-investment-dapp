...
module.exports = async function (callback) {
  try {
    ...
    await dp.approve(vault.address, 500, { from: accounts[9] });
    await dp.approve(vault.address, 300, { from: accounts[8] });
    await dp.approve(vault.address, 100, { from: accounts[7] });
    console.log("DP approved for the vault");

    // Withdraw DP from the vault
    await vault.withdraw(500, { from: accounts[9] });
    await vault.withdraw(300, { from: accounts[8] });
    await vault.withdraw(100, { from: accounts[7] });
